...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    function searchPrev(Tree storage self, uint key) internal view returns (uint256) {
        uint cursor = self.root;
        while (cursor > key && self.nodes[cursor].left != EMPTY) {
            cursor = self.nodes[cursor].left;
        }
        if (self.nodes[cursor].left == EMPTY && cursor > key) {
            return 0;
        }
        if (cursor == key) {
            return cursor;
        }
        return treeMaximum(self, cursor);
    }
}
// ----------------------------------------------------------------------------
